...
fn roundedBoxSDF(p: vec2<f32>, size: vec2<f32>, r: vec4<f32>) -> f32 {
  ...
  // 根据象限选择对应的圆角半径
  if (p.x > 0.0) {
    radius = vec4<f32>(radius.x, radius.y, radius.x, radius.y);
  } else {
    radius = vec4<f32>(radius.w, radius.z, radius.w, radius.z);
  }
  let cornerRadius = select(radius.y, radius.x, p.y > 0.0);
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
  ...
  // 计算整体节点的 SDF
  let nodeRadius = vec4<f32>(radius, radius, radius, radius);
  let nodeDist = roundedBoxSDF(centerPos, halfSize, nodeRadius);
